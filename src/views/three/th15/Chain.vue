...
export default {
  ...
  methods: {
    ...
    initModel() {
      ...
      //直接链式实现tween
      this.tween = new TWEEN.Tween(cube.position).to(
        { x: 40, y: 30, z: 30 },
        2000
      );

      let tween2 = new TWEEN.Tween(cube.position).to(
        { x: 40, y: 30, z: -30 },
        2000
      );

      let tween3 = new TWEEN.Tween(cube.position).to(
        { x: -40, y: 0, z: -30 },
        2000
      );
